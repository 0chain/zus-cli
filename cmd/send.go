...
func checkBalanceBeforeSend(tokens, fee uint64) {
	...
	if uint64(b.Balance) < tokens+fee {
		util.ExitWithError("Insufficient balance for this transaction.")
	}
	return
}
...
var sendcmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if fflags.Changed("to_client_id") == false {
			util.ExitWithError("Error: to_client_id flag is missing")
		}
		if fflags.Changed("tokens") == false {
			util.ExitWithError("Error: tokens flag is missing")
		}
		if fflags.Changed("desc") == false {
			util.ExitWithError("Error: Description flag is missing")
		}
		tokenZCN, err := cmd.Flags().GetFloat64("tokens")
		if err != nil {
			util.ExitWithError("Error: invalid 'tokens' flag", err)
		}
		if tokenZCN < 0 {
			util.ExitWithError("invalid tokens amount: negative")
		}
		...
		if err != nil {
			util.ExitWithError("Send tokens failed. " + err.Error())
		}
		if doJSON {
			j := map[string]string{
				"status": "success",
