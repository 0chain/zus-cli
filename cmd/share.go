...
var shareCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			util.PrintError("Error fetching the file information", err)
			os.Exit(1)
		}
		...
		if isFile {
			...
			if err != nil {
				util.PrintError("Error in getting the file meta of the object." + err.Error())
				os.Exit(1)
			}

			if len(fileMeta.EncryptedKey) > 0 && fflags.Changed("encryptionpublickey") == false {
				util.PrintError("Clientid and/or encryptionpublickey are missing for the encrypted share!")
				os.Exit(1)
			}
		}
