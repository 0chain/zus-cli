...
var getallocationCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("    read_price: ", calculateDownloadCost(alloc, (util.GB+fileref.CHUNK_SIZE-1)/fileref.CHUNK_SIZE), "/ GB (by 64KB)")
...
func uploadCostFor1GB(alloc *sdk.Allocation) (cost common.Balance) {
	for _, d := range alloc.BlobberDetails {
		cost += uploadCostForBlobber(float64(d.Terms.WritePrice), 1*util.GB,
			alloc.DataShards)
	}
	return
}
