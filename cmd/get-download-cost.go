...
func calculateDownloadCost(alloc *sdk.Allocation, numBlocks int64) common.Balance {
	...
	for _, d := range alloc.BlobberDetails {
		...
		cost += util.SizeInGB(numBlocks*fileref.CHUNK_SIZE) * float64(readPrice)
...
func downloadCost(alloc *sdk.Allocation, meta *sdk.ConsolidatedFileMeta) {
	...
	requiredBalance := calculateDownloadCost(alloc, numBlocks)
...
func init() {
	...
	// CHECK: blocks-per-marker is unused
